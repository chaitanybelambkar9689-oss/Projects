...
int main()
{
    ...
    ServerAddr.sin_family = AF_INET;          // connect with  Intenet protocall
    ServerAddr.sin_addr.s_addr = INADDR_ANY;  // server  kontyahi interface varun yenari connection swikaru shakto
    ServerAddr.sin_port = htons(Port);       // port number set kela htons() = host to network
